...
Array.prototype.contains = function (arr){	
 ...
 function save(){
	   ...
	   var userIds;
	   var userNames;
	   if(opener.$('#receivers').val() == null || opener.$('#receivers').val() == ""){
		   userIds = new Array();
		   userNames = new Array();
	   }else{
		   userIds = opener.$('#receivers').val().split(',');
		   userNames = opener.$('#receiverNames').val().split(',');
	   }
	   for(var i = 0; i < len; i++){
		   if(chk[i].checked){
			   if(!userIds.contains(chk[i].value)){
				   userIds.push(chk[i].value);
				   userNames.push(chk[i].alt);
			   }
		   }
	   }
	   if(userIds.length == 0){
	   ...
	   opener.$('#receivers').val(userIds.join(","));
	   opener.$('#receiverNames').val(userNames.join(","));
 	   window.close();
}
 qurMembers();
</script>