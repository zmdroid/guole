				...
				<div class="con">
					<table class="display stylized" width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							...
							<td class="textLeft">
								<c:if test="${billVO.tradechannel == 1}">支付宝 </c:if>
								<c:if test="${billVO.tradechannel == 2}">线下汇款</c:if>
								<c:if test="${billVO.tradechannel == 3}">银行转账</c:if>
								<c:if test="${billVO.tradechannel == 4}">平台内</c:if>
							</td>
							<td class="textLeft">
